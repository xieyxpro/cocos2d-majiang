...
local GamePlayer = require("lua.RoomServer.gameai.GamePlayer")
...
local GameCache = Util:class("GameCache")

local seatDirMap = {
    [4] = {GameDefine.DIR_BOTTOM, GameDefine.DIR_LEFT, GameDefine.DIR_TOP, GameDefine.DIR_RIGHT},
    [3] = {GameDefine.DIR_BOTTOM, GameDefine.DIR_LEFT, GameDefine.DIR_RIGHT},
...
function GameCache:__init(params)
    self.event = params.event
    self.myself = params.myself
    ...
    self.event:register("ms_create_room", self, "ms_create_room")
    self.event:register("ms_join_room", self, "ms_join_room")
    self.event:register("ms_room_info", self, "ms_room_info")
    ...
    self.event:register("ms_room_players_info", self, "ms_room_players_info")
    self.event:register("ms_sit_down", self, "ms_sit_down")
    self.event:register("ms_game_scene_free", self, "ms_game_scene_free")
    self.event:register("ms_game_scene_play", self, "ms_game_scene_play")
    self.event:register("ms_user_offline", self, "ms_user_offline")
    self.event:register("ms_user_reconnect", self, "ms_user_reconnect")    
    self.event:register("mc_ms_talk", self, "mc_ms_talk")
    self.event:register("ms_location", self, "ms_location")
...
--    self.event:register("ms_leave_room", self, "ms_leave_room")
    self.event:register("ms_player_ready", self, "ms_player_ready")
    self.event:register("ms_player_ting", self, "ms_player_ting")
    self.event:register("ms_haidilao", self, "ms_haidilao")

    self.event:register("ms_stand_up", self, "ms_stand_up")
    self.event:register("ms_dismiss", self, "ms_dismiss")
    self.event:register("ms_dismiss_confirm", self, "ms_dismiss_confirm")
    self.event:register("ms_dismiss_fail", self, "ms_dismiss_fail")

    self.event:register("voice_upload_finish",self,"voice_upload_finish")
end 
...
function GameCache:__updateSeatDirOf(player)
    if player.userid == self.myself.userid then 
        player.seatDir = GameDefine.DIR_BOTTOM
        return
    end 
    local myself = self.players[self.myself.userid]
...
function GameCache:getGangs()
    local gangs = {}
    local player = self.players[self.myself.userid]
...
function GameCache:ms_create_room(data)
    self.event:dispatch("MS_CREATE_ROOM", data)
end

function GameCache:ms_join_room(data)
    self.event:dispatch("MS_JOIN_ROOM", data)
end
...
function GameCache:ms_room_info(data)
    ...
    self.event:dispatch("MS_ROOM_INFO", data)
end 
...
function GameCache:ms_game_scene_free(data)
    ...
    self.event:dispatch("MS_GAME_SCENE_FREE", data)
end

function GameCache:ms_room_players_info(data)
    for _, playerData in pairs(data.players) do 
        local player = GamePlayer:create(self, playerData)
...
--        end 
    ...
    self.event:dispatch("MS_ROOM_PLAYERS_INFO", data)
end
...
--    end 
    ...
    self:checkOthersCheating()
    self.event:dispatch("MS_SIT_DOWN", data)
end
...
function GameCache:ms_game_scene_play(data)
    ...
    self.event:dispatch("MS_GAME_SCENE_PLAY", data)
end 

function GameCache:ms_stand_up(data)
    self.players[data.userid] = nil 
    self.event:dispatch("MS_STAND_UP", data)
end 
...
function GameCache:voice_upload_finish(data)
    if data.bSuccess then
        Network:send(Define.SERVER_GAME,"mc_ms_talk",{userid = self.myself.userid,talkurl=data.fileurl,time=data.time})
        return
    end
...
function GameCache:ms_user_reconnect(data)
    ...
    self.event:dispatch("MS_USER_RECONNECT", data)
end 

function GameCache:ms_user_offline(data)
    self.players[data.userid].isOffline = true
    self.event:dispatch("MS_USER_OFFLINE", data)
end 
...
function GameCache:ms_game_ready(data)
    ...
    player.ready = true
    self.event:dispatch("MS_GAME_READY", data)
end 
...
function GameCache:ms_system_dispatch_card(data)
    ...
    self.event:dispatch("MS_SYSTEM_DISPATCH_CARD", data)
end 
...
function GameCache:ms_out_card(data)
    ...
    self.event:dispatch("MS_OUT_CARD", data)
end 
...
function GameCache:ms_action_guo(data)
    ...
    self:__resetActions()
    self.event:dispatch("MS_ACTION_GUO", data)
end
...
function GameCache:ms_action_peng(data)
    ...
    self.event:dispatch("MS_ACTION_PENG", data)
end
...
function GameCache:ms_action_hu(data)
    ...
    for _, player in pairs(self.players) do 
        player.status = GameDefine.enum_UserStatus.US_FREE
    end 
    self:__resetActions()
    self.event:dispatch("MS_ACTION_HU", data)
end
...
function GameCache:ms_action_gang(data)
    ...
    self.event:dispatch("MS_ACTION_GANG", data)
end
...
function GameCache:ms_action_chi(data)
    ...
    self.event:dispatch("MS_ACTION_CHI", data)
end
...
function GameCache:ms_haidilao(data)
    ...
    --distrubute
    self.event:dispatch("MS_HAIDILAO", data)
end
...
function GameCache:ms_game_start(data)
    ...
    for _, rollPlayerData in pairs(data.players) do 
        assert(self.players[rollPlayerData.userid] ~= nil, string.format("player %d join room first", rollPlayerData.userid))
        local player = self.players[rollPlayerData.userid]
        player.status = GameDefine.enum_UserStatus.US_PLAYING
        player.huType = rollPlayerData.huType or 0
        player.handCards = {} --clear
        player.handCardsNum = rollPlayerData.handCardsNum or 0
        player.isZhuang = rollPlayerData.userid == data.zhuangID
    ...
    self.event:dispatch("MS_GAME_START", data)
end
...
function GameCache:ms_game_over(data)
    if not data.err or data.err == 0 then 
        ...
        for k, playerData in ipairs(data.players) do 
            ...
            player.score = player.score + balancePlayer.record
        end 
    ...
    self.event:dispatch("MS_GAME_OVER", data)
end
...
function GameCache:ms_player_ready(data)
    ...
    player.status = GameDefine.enum_UserStatus.US_READY
    self.event:dispatch("MS_PLAYER_READY", data)
end
...
function GameCache:ms_player_ting(data)
    ...
    self.event:dispatch("MS_PLAYER_TING", data)
end
...
function GameCache:ms_stand_up(data)
    ...
    self.players[data.userid] = nil
    if data.userid == self.myself.userid then 
        --TODO clear cache
    end 
    self.event:dispatch("MS_STAND_UP", data)
end
...
function GameCache:ms_dismiss(data)
    --TODO clear cache
    self.event:dispatch("MS_DISMISS", data)
end
...
function GameCache:ms_dismiss_confirm(data)
    ...
    self.event:dispatch("MS_DISMISS_CONFIRM", data)
end

function GameCache:ms_dismiss_fail(data)
    self.dismiss = nil
    self.event:dispatch("MS_DISMISS_FAIL", data)
end
...
function GameCache:ms_game_record(data)
    ...
    self.event:dispatch("MS_GAME_RECORD", statistics)
end
...
function GameCache:ms_prev_dismiss(data)
    ...
    self.event:dispatch("MS_PREV_DISMISS", data)
end
