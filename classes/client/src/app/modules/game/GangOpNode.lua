...
--Date
--此文件由[BabeLua]插件自动生成
local GangOpNode = class("GangOpNode", cc.Node)
...
function GangOpNode:setMode(mode)
    self.mode = mode 
end 

function GangOpNode:addCards(mingCards)
...
function GangOpNode:addCard(cardVal, mingType, subMingType)
...
function GangOpNode:rmvCard(cardVal, mingType, subMingType)
    mingType = mingType or 0
    subMingType = subMingType or 0
...
function GangOpNode:createMingCard(cardVal, mingType, subMingType)
    local node = nil
...
function GangOpNode:refresh()
    for i, mingCard in ipairs(self.mingCards) do 
        if not mingCard.node then 
            ...
            mingCard.node:addTouchEventListener(function(sender, event)
                ...
                if event == ccui.TouchEventType.ended then 
                    local data = {}
                    data.cardVal = mingCard.cardVal
                    data.mingType = mingCard.mingType
                    data.subMingType = mingCard.subMingType
