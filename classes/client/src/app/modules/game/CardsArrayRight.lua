...
function CardsArrayRight:setMode(mode)
    self.mode = mode 
end 

function CardsArrayRight:incCardsNum(num)
    for i = 1, num, 1 do 
...
function CardsArrayRight:decCardsNum(num)
    for i = 1, num, 1 do 
...
function CardsArrayRight:addCard(cardVal)
    self:__resetSysCard()
...
function CardsArrayRight:addSysCard(cardVal)
    ...
    card.node:setPosition(cc.p(selfSz.width * anpt.x, selfSz.height + self.sysCardIntvl + contentSz.height * anpt.y))
...
function CardsArrayRight:__resetSysCard()
    if self.mode == GameDefine.GAME_MODE.GAME then 
        return
    end 
...
function CardsArrayRight:rmvCard(cardVal)
    self:__resetSysCard()
...
function CardsArrayRight:addCards(cards)
    self:__resetSysCard()
...
function CardsArrayRight:rmvCards(cards)
    self:__resetSysCard()
    local tmp = {}
    for k, v in pairs(cards) do 
...
function CardsArrayRight:createCardNode(cardVal)
    local node 
    if self.mode == GameDefine.GAME_MODE.GAME then 
        node = cc.Sprite:create("GameScene/hand_right.png")
...
function CardsArrayRight:refresh()
    for i, card in ipairs(self.cards) do 
...
function CardsArrayRight:getSysCardPosition()
    local pos = cc.p(self:getPosition())
    local nodeSz = self:createCardNode(11):getContentSize()
    pos.y = pos.y - nodeSz.height * 0.5
    return self:getParent():convertToWorldSpace(pos)
end 

function CardsArrayRight:getSysCardSize()
    local nodeSz = self:createCardNode(11):getContentSize()
    return {
...
function CardsArrayRight:selectAOutCardNode(cardVal)
