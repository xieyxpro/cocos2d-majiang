...
一个简单的富文本 Label，用法
    ...
    txt:setText("<#333>你\t好<#800>\n\t&lt;世界&gt;<img GameScene/vertical/mingmah_46.png><img_50*50 GameScene/vertical/mingmah_46.png><font font/DFYuanW7-GB2312.ttf><#555><64>Big<#077><18>SMALL<")

--    local text = ComptMgr.createXMLRichText():addTo(self):move(500,320)
--    text:setContentSize(cc.size(400,0))
--    text:setMultiLineMode(true)
--    text:setText("每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>每天可购买体力10次<line 10>")
	...
	txt:setContentSize(200, 400)    --当设置高度给0，则自动调整高度，formatText后获取正确高度
...
基本选项是
    ...
    <line height>       = 插入height高度的行
	<+2> <-2> <*2> </2> = 当前字体大小 +-*/
	<!>					= 颜色、字体和字体大小恢复默认
	\n \t 				= 换行 和 tab，可能暂时实现得不是很好
    <outLine 1>         = 设置1像素描边，只支持TTF字体
    <outLineColor #FFFFFF> = 设置描边颜色，默认为黑色
...
--/////////////////////////////////////////////////////////////////////////////
function RichTextEx:ctor(fontSize, textColor,fontName)
	self._text			= ""
	self._fontSizeDef	= fontSize or 22
	self._textColorDef	= textColor or cc.c3b(255, 255, 255)
    self._fontNameDef = fontName or Define.FONT_NAME
	self._fontSize		= self._fontSizeDef
	self._textColor		= self._textColorDef
	self._elements		= {}
    self._textFont      = self._fontNameDef
	self._outLine 		= 0
    self._outLineColor  = cc.c4b(0,0,0,255)
...
function RichTextEx:setMultiLineMode(b)
	self:ignoreContentAdaptWithSize(not b)
	return self
end

--/////////////////////////////////////////////////////////////////////////////
function RichTextEx.defaultCb(text, sender)
	local BLINK		= "blink "
	local ROTATE	= "rotate "
	local SCALE		= "scale "
...
--<img filename>		= 图片（filename 可以是已经在 SpriteFrameCache 里的 key，或磁盘文件）
--<img_32*32 fname> 	= 指定大小的图片
function RichTextEx.defaultImgCb(text)
	local w, h = 0, 0
...
--<line height>		= 行高
function RichTextEx:newLine(height)
    height = height or self._fontSize
    local obj = cc.Node:create()
	obj:setContentSize(cc.size(self:getContentSize().width, height))
    return obj
end

--/////////////////////////////////////////////////////////////////////////////
function RichTextEx:addCustomNode(node)
	if node then
	...
	while i <= len do
		...
		if c == P_BEG then	-- <
			if (not b) and (i > p) then
				...
				obj = ccui.RichElementText:create(0, self._textColor, 255, self:htmlDecode(str), self._textFont, self._fontSize)--TODO,self._outLine,self._outLineColor, self._underLine)
				self:pushBackElement(obj)
				self._elements[#self._elements + 1] = obj
			end
				...
				if str_byte(text, i) == P_END then	-- >
					...
					if i > p then
						...
						elseif chr == C_RST and #str == 1 then	-- reset
							...
							self._textFont  = self._fontNameDef
							self._outLine = 0
                            self._outLineColor = cc.c4b(0,0,0,0)
							self._underLine = false 
						elseif (chr == C_INC or chr == C_DEC or chr == CRichTextExUL or chr == C_DIV)
                        ...
                        elseif str_sub(str, 1, 13) == "outLineColor " or str_sub(str,1,13) == "outLineColor_" then
                            if (#str == 17 or #str == 20) and tonumber(str_sub(str, 15), 16) then
                                self._outLineColor = c3b_to_c4b(c3b_parse(str_sub(str, 14)))
                            end
                        ...
                        elseif str_sub(str, 1, 5) == "line " or str_sub(str,1, 5) == "line_" then
                            obj = ccui.RichElementNewLine:create(0, self._textColor, 255)
                            self:pushBackElement(obj)
		                    self._elements[#self._elements + 1] = obj

                            self:addCustomNode(self:newLine(str_trim(str_sub(str, 5, i - 1))))
...
function RichTextEx:setDefaultFont(font)
	self._textFont = font
end
...
function RichTextEx.htmlEncode(self,input)
...
function RichTextEx.htmlDecode(self,input)
	if not input then input = self end
    input = string.gsub(input,"&gt;",">")
    input = string.gsub(input,"&lt;","<")
    input = string.gsub(input,"&#039;","'")
...
function RichTextEx:create(...)
	local richTextEx = RichTextEx.new(...)
    return richTextEx
end
